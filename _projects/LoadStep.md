---
layout: project
title: LoadStep
description: 
technologies: [MATLAB, Simulink, Latex]
image: /assets/images/BAE_Malloy_T150_Heavy_Lift_UAS.png
---


## Project Details

**Course:** MAE, Addvanced Product Design, Spring 2026 <br>
**Institution:** Cornell University <br>
**Tools:**  <br>
**Analysis Methods:**<br>
**Deliverables:** Full Technical Report Below <br>
<embed src="{{ '/assets/PDS Draft for Completion.pdf' | relative_url }}" type="application/pdf" width="100%" height="600px" />


---

